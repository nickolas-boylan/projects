        ...
        window.onload = function() {
            ...
            function savePal() {
                ...
                $.ajax({
                    url : "save.php",
                    method : "GET",
                    data: {name: pal_name, type: "ui", color0: color0, color1: color1, color2: color2, color3: color3},
            ...
            // Function to show the saved message
            function showSave() {
                popup = $("#save_popup");
                popup.css("display", "flex");
                $("#shade").show();

                // Set timeout to fade out message and backdrop
                setTimeout(function() {
                    popup.fadeOut();
                    $("#shade").fadeOut();
                    $("#palette-name").val("");
            ...
            // Show the popup box with errors
            function showPopup() {
                $("#popup").css("display", "flex");
                $("#shade").show();
            }
            ...
            window.logout = function logout() {
                $.ajax({
                    url : "logout.php",
                    success: setTimeout(function() {window.location.replace("ui.php")}, 100),
                    error: function(xhr) {
                        console.log(xhr.status + " " + xhr.statusText)
                    }
                });
...
<div class="shade" id='shade'></div>
    ...
    <div class="main-container">
        ...
        <?php
            if (!isset($_SESSION["username"]) || !isset($_SESSION["premium"])) {
                die("You must be logged in to a premium account to use this page");
            } else if (!$_SESSION["premium"]) {
                die("This feature is locked to only premium accounts. If you wish to access it, sign up now for a subscription to Coluux+");
            }
        ?>
            <div class="body-top">
